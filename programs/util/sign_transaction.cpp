...
{
   scorum::protocol::transaction     tx;
   std::string                        wif;
};
...
struct tx_signing_result
{
   ...
   scorum::protocol::public_key_type key;
   scorum::protocol::signature_type  sig;
};
...
int main(int argc, char** argv, char** envp)
{
   ...
   while( std::cin )
   {
      ...
      sres.key = scorum::protocol::public_key_type( priv_key.get_public_key() );
