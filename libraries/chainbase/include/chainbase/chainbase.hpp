...
public:
    ...
    void squash()
    {
        ...
        // object. Type B means the composition of states contains the same entry as the second of the two merged states
        // for that object. Type C means the composition of states contains an entry different from either of the merged
        // states for that object. Type N/A means the composition of states violates causal timing. Type AB means both
        // type A and type B simultaneously.
        //
        // The merge() operation is defined as modifying prev_state in-place to be the state object which represents the
        // composition of state A and B.
        //
        // Type A (and AB) can be implemented as a no-op; prev_state already contains the correct value for the merged
        // state. Type B (and AB) can be implemented by copying from state to prev_state. Type C needs special
        // case-by-case logic. Type N/A can be ignored or assert(false) as it can only occur if prev_state and state
        // have illegal values (a serious logic error which should never happen).
        //
