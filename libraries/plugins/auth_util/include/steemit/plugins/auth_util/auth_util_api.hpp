...
FC_REFLECT( scorum::plugin::auth_util::check_authority_signature_params,
   (account_name)
   (level)
   (dig)
   (sigs)
   )
FC_REFLECT( scorum::plugin::auth_util::check_authority_signature_result,
   (keys)
   )

FC_API( scorum::plugin::auth_util::auth_util_api,
