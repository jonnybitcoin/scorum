...
template <typename A, typename B> erpair<A, B> make_erpair(const A& a, const B& b) { return erpair<A, B>(a, b); }

template <typename T> T endian_reverse(const T& x) { return boost::endian::endian_reverse(x); }
...
inline void pack(Stream& s, const scorum::protocol::fixed_string<Storage>& u)
{
    pack(s, std::string(u));
}

template <typename Stream, typename Storage> inline void unpack(Stream& s, scorum::protocol::fixed_string<Storage>& u)
{
    std::string str;
    unpack(s, str);
    u = str;
...
template <typename Storage> void to_variant(const scorum::protocol::fixed_string<Storage>& s, variant& v)
{
    v = std::string(s);
}

template <typename Storage> void from_variant(const variant& v, scorum::protocol::fixed_string<Storage>& s)
{
    s = v.as_string();
}
} // namespace fc
