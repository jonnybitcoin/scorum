...
share_type calc_percent_reward(share_type current_supply)
{
    ...
    static_assert(percent <= SCORUM_100_PERCENT,
        "percent is implausibly large, re-check your arguments (if you really mean to do this, you should revise the "
        "overflow check above accordingly)");
...
{
    return calc_percent_reward<percent, SCORUM_APR_PERCENT_MULTIPLY_PER_HOUR, SCORUM_APR_PERCENT_SHIFT_PER_HOUR>(
        current_supply);
}

template <uint16_t percent> inline share_type calc_percent_reward_per_block(share_type current_supply)
{
    return calc_percent_reward<percent, SCORUM_APR_PERCENT_MULTIPLY_PER_BLOCK, SCORUM_APR_PERCENT_SHIFT_PER_BLOCK>(
        current_supply);
}

template <uint16_t percent> inline share_type calc_percent_reward_per_round(share_type current_supply)
{
    return calc_percent_reward<percent, SCORUM_APR_PERCENT_MULTIPLY_PER_ROUND, SCORUM_APR_PERCENT_SHIFT_PER_ROUND>(
        current_supply);
}
