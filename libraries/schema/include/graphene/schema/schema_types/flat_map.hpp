...
template <typename K, typename V> struct schema_flat_map_impl : public abstract_schema
{
...
template <typename K, typename V> void schema_flat_map_impl<K, V>::get_str_schema(std::string& s)
{
    if (str_schema != "")
    {
        s = str_schema;
    ...
    mvo("name", my_name)("type", "map")("ktype", k_name)("vtype", v_name);
