...
template <typename Storage> struct schema_fixed_string_impl : public abstract_schema
{
...
template <typename Storage> void schema_fixed_string_impl<Storage>::get_str_schema(std::string& s)
{
    if (str_schema != "")
    {
        s = str_schema;
...
template <typename Storage> struct schema_reflect<typename fc::fixed_string<Storage>>
{
