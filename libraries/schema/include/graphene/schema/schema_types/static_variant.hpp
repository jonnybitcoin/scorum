...
template <typename... Types> struct schema_static_variant_impl : public abstract_schema
{
...
template <> struct get_schemas_for_types_impl<>
{
    static void get(std::vector<std::shared_ptr<abstract_schema>>& schemas) {}
};

template <typename T, typename... Types> struct get_schemas_for_types_impl<T, Types...>
{
    static void get(std::vector<std::shared_ptr<abstract_schema>>& schemas)
    {
...
template <typename... Types> void get_schemas_for_types(std::vector<std::shared_ptr<abstract_schema>>& schemas)
{
    get_schemas_for_types_impl<Types...>::get(schemas);
    return;
}
...
template <typename... Types> void schema_static_variant_impl<Types...>::get_str_schema(std::string& s)
{
    if (str_schema != "")
    {
        s = str_schema;
    ...
    mvo("name", my_name)("type", "static_variant")("etypes", e_types);
...
template <typename... Types> struct schema_reflect<fc::static_variant<Types...>>
{
