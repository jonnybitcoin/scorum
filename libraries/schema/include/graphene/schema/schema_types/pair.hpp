...
template <typename K, typename V> struct schema_pair_impl : public abstract_schema
{
...
template <typename K, typename V> void schema_pair_impl<K, V>::get_str_schema(std::string& s)
{
    if (str_schema != "")
    {
        s = str_schema;
    ...
    mvo("name", my_name)("type", "tuple")("etypes", e_types);
...
template <typename K, typename V> struct schema_reflect<std::pair<K, V>>
{
