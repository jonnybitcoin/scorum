...
template <typename ObjectType, bool is_reflected = fc::reflector<ObjectType>::is_defined::value,
    bool is_enum = fc::reflector<ObjectType>::is_enum::value>
struct schema_impl;

} // namespace detail

template <typename ObjectType> struct schema_reflect
{
    typedef detail::schema_impl<ObjectType> schema_impl_type;
};

template <typename ObjectType> std::shared_ptr<abstract_schema> get_schema_for_type()
{
