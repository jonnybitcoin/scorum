...
{
    get_deps_member_visitor(std::vector<std::shared_ptr<abstract_schema>>& deps)
        : _deps(deps)
    {
    }
...
struct get_str_schema_enum_member_visitor
{
    ...
    template <typename Enum> void operator()(const char* name, Enum value) const
    {
...
#define GRAPHENE_DECLARE_SCHEMA_CLASS(is_reflected, is_enum)                                                           \
    template <typename ObjectType> struct schema_impl<ObjectType, is_reflected, is_enum> : public abstract_schema      \
    {                                                                                                                  \
...
#define GRAPHENE_SCHEMA_CLASS_BODY(CLASSNAME)                                                                          \
    ...
    virtual ~CLASSNAME() {}                                                                                            \
                                                                                                                       \
    virtual void get_deps(std::vector<std::shared_ptr<abstract_schema>>& deps) override;                               \
    virtual void get_name(std::string& name) override { name = _name; }                                                \
    virtual void get_str_schema(std::string& s) override;                                                              \
    virtual int64_t get_id() override { return _id; }                                                                  \
                                                                                                                       \
...
template <typename ObjectType> void schema_impl<ObjectType, false, false>::get_str_schema(std::string& s)
{
    if (str_schema != "")
    {
        s = str_schema;
...
template <typename ObjectType> void schema_impl<ObjectType, true, false>::get_str_schema(std::string& s)
{
    if (str_schema != "")
    {
        s = str_schema;
    ...
    get_name(my_name);
    fc::mutable_variant_object mvo;
    mvo("name", my_name)("type", "class")("members", vtor._members);
...
template <typename ObjectType> void schema_impl<ObjectType, true, true>::get_str_schema(std::string& s)
{
    if (str_schema != "")
    {
        s = str_schema;
    ...
    get_name(my_name);
    fc::mutable_variant_object mvo;
    mvo("name", my_name)("type", "enum")("members", vtor._members);
...
template <typename ObjectType> std::shared_ptr<abstract_schema> create_schema(int64_t id)
{
    return std::make_shared<typename schema_reflect<ObjectType>::schema_impl_type>(
        id, fc::get_typename<ObjectType>::name());
}
