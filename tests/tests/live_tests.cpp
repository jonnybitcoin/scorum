...
BOOST_AUTO_TEST_CASE( vests_stock_split )
{
   try
   {
      ...
      auto old_vesting_fund = db.get_dynamic_global_properties().total_vesting_fund_scorum;
      ...
      auto old_reward_fund = db.get_dynamic_global_properties().total_reward_fund_scorum;
...
com_itr->net_rshares.value * magnitude * magnitude : 0; u256 rs( com_itr->net_rshares.value ); u256 rf(
gpo.total_reward_fund_scorum.amount.value ); auto rs2 = rs * rs; u256 rshares2 = old_rshares2.hi; rshares2 = rshares2 <<
64; rshares2 += old_rshares2.lo; expected_reward[ com_itr->id ] = static_cast< uint64_t >( rf * rs2 / rshares2 );
         }
         com_itr++;
      }
...
magnitude ); BOOST_REQUIRE( db.get_dynamic_global_properties().total_reward_shares2 == total_rshares2 ); BOOST_REQUIRE(
db.get_dynamic_global_properties().total_reward_fund_scorum == old_reward_fund );
...
) ] * magnitude ); BOOST_REQUIRE( com_itr->abs_rshares == comment_abs_rshares[ std::make_tuple( com_itr->author,
com_itr->permlink ) ] * magnitude ); BOOST_REQUIRE( com_itr->total_vote_weight == total_vote_weights[ com_itr->id ] );
